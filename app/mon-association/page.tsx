          ...
          {memberships.length ? memberships.map(({ role, associations }) => {
            ...
            <article key={association.id} className="rounded-3xl border border-ink-200 bg-white p-6 shadow-card sm:p-8">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {association.status === 'approved' ? <><Link href={`/associations/${association.slug}`} className="rounded-xl border border-ink-200 px-4 py-2.5 text-sm font-medium">Voir la page publique</Link><Link href="/mes-tournois" className="inline-flex items-center gap-2 rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-medium text-white"><Trophy className="h-4 w-4" /> Mes tournois</Link></> : <span className="inline-flex items-center gap-2 text-sm text-ink-500"><Clock3 className="h-4 w-4" /> La création de tournoi sera activée après validation.</span>}
              </div>
            </article>
          ) : null;
          }) : (
