...
export default async function TournamentPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ erreur?: string; message?: string }> }) {
  ...
  const { data, error } = await supabase.from('tournaments').select(`
    ...
    participants:tournament_participants(profile_id, profile:profiles(id, username, first_name, last_name, show_real_name, avatar_path)),
    likes:tournament_likes(profile_id),
    comments:tournament_comments(id, author_id, body, created_at, author:profiles!tournament_comments_author_id_fkey(id, username, first_name, last_name, show_real_name, avatar_path))
  ...
  const [{ data: linkedPosts }, { data: officialResults }, claimResult] = await Promise.all([
    supabase.from('posts').select(FEED_SELECT).eq('tournament_id', tournament.id).eq('status', 'published').is('deleted_at', null).order('created_at', { ascending: false }).limit(12),
    supabase.from('tournament_results').select('id, team_name, position, result_label').eq('tournament_id', tournament.id).order('position', { ascending: true }),
    identity ? supabase.from('tournament_result_claims').select('id, claimed_position, claimed_label, note, status').eq('tournament_id', tournament.id).eq('profile_id', identity.id).maybeSingle() : Promise.resolve({ data: null }),
  ]);
  const resultClaim = claimResult.data;
  const info = [
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-7 lg:grid-cols-[390px_minmax(0,1fr)] lg:gap-11">
        <div className="lg:sticky lg:top-24 lg:self-start">
          <div className="relative aspect-[4/5] overflow-hidden rounded-3xl bg-ink-100 shadow-lift"><Image src={protectedMediaUrl(tournament.poster_url) ?? ''} alt={`Affiche de ${tournament.name}`} fill sizes="(max-width: 1024px) 100vw, 390px" className="object-cover" priority unoptimized /></div>
        ...
        <div className="min-w-0 space-y-6">
          ...
          {(officialResults?.length || participates) && <section className="rounded-3xl border border-ink-200 bg-gradient-to-br from-white to-amber-50/40 p-6 shadow-soft"><h2 className="font-display text-xl font-semibold">Résultats et palmarès</h2>{officialResults?.length ? <div className="mt-4 space-y-2">{officialResults.map((result) => <div key={result.id} className="flex items-center gap-3 rounded-xl bg-white p-3 ring-1 ring-ink-100"><strong className="w-10 text-center font-display text-xl">{result.position ? `${result.position}e` : '—'}</strong><span className="text-sm"><strong className="block">{result.team_name}</strong>{result.result_label}</span></div>)}</div> : <p className="mt-3 text-sm text-ink-500">Aucun classement officiel publié.</p>}{participates && <form action={submitResultClaim} className="mt-5 grid gap-3 border-t border-ink-100 pt-5 sm:grid-cols-2"><input type="hidden" name="tournament_id" value={tournament.id} /><label className="text-xs font-medium">Place<input name="claimed_position" type="number" min="1" defaultValue={resultClaim?.claimed_position ?? ''} className="mt-1.5 w-full rounded-xl border border-ink-200 px-3 py-2.5" /></label><label className="text-xs font-medium">Résultat libre<input name="claimed_label" defaultValue={resultClaim?.claimed_label ?? ''} placeholder="Ex. Finaliste" className="mt-1.5 w-full rounded-xl border border-ink-200 px-3 py-2.5" /></label><label className="text-xs font-medium sm:col-span-2">Note pour l’organisateur<textarea name="note" defaultValue={resultClaim?.note ?? ''} rows={2} className="mt-1.5 w-full rounded-xl border border-ink-200 px-3 py-2.5" /></label><div className="flex items-center justify-between sm:col-span-2">{resultClaim && <span className="text-xs font-semibold text-ink-500">Statut : {resultClaim.status}</span>}<button className="ml-auto rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-semibold text-white">Soumettre pour confirmation</button></div></form>}</section>}
          <section id="commentaires" className="scroll-mt-24 rounded-3xl border border-ink-200 bg-white p-5 shadow-soft sm:p-7"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 font-display text-xl font-semibold"><MessageCircle className="h-5 w-5" /> Commentaires</h2><span className="text-xs text-ink-400">{tournament.comments.length}</span></div><div className="mt-5 space-y-4">{tournament.comments.map((comment) => { const author = one(comment.author); return <article key={comment.id} className="flex gap-3"><ProfileAvatar username={author?.username} avatarPath={author?.avatar_path} /><div className="min-w-0 flex-1 rounded-2xl bg-ink-50 px-4 py-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-semibold">{displayName(author)}</p>{identity?.id === comment.author_id && <form action={deleteTournamentComment}><input type="hidden" name="tournament_id" value={tournament.id} /><input type="hidden" name="comment_id" value={comment.id} /><button className="text-[11px] text-ink-400 hover:text-red-600">Supprimer</button></form>}</div><p className="mt-1 text-sm leading-6 text-ink-600">{comment.body}</p></div></article>; })}</div>{identity && <form action={addTournamentComment} className="mt-5 flex gap-2 border-t border-ink-100 pt-5"><input type="hidden" name="tournament_id" value={tournament.id} /><label className="sr-only" htmlFor="tournament-comment">Ton commentaire</label><input id="tournament-comment" name="body" required maxLength={2000} placeholder="Ajouter un commentaire…" className="min-w-0 flex-1 rounded-xl border border-ink-200 px-4 py-3 text-base sm:text-sm" /><button className="rounded-xl bg-ink-950 px-4 py-3 text-sm font-semibold text-white">Envoyer</button></form>}</section>
        </div>
      </div>
      <section className="mt-12"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-reunion-blue">Archive vivante</p><h2 className="mt-2 font-display text-3xl font-semibold">Souvenirs du tournoi</h2><p className="mt-2 text-sm text-ink-500">Publications, photos et échanges liés à cet événement.</p></div><div className="mt-6 mx-auto max-w-3xl space-y-5">{(linkedPosts as unknown as FeedPost[] | null)?.length ? (linkedPosts as unknown as FeedPost[]).map((post) => <FeedPostCard key={post.id} post={post} identityId={identity?.id ?? ''} />) : <div className="rounded-3xl border border-dashed border-ink-300 bg-white p-8 text-center text-sm text-ink-500">Aucun souvenir publié pour le moment. Lie une publication à ce tournoi depuis le Feed.</div>}</div></section>
    </div>
  </main>;
}
