...
export default function AddressAutocomplete() {
  ...
  function choose(place: Place) { setSelected(place); setQuery(place.display_name); setResults([]); inputRef.current?.setCustomValidity(''); }

  return <div className="relative sm:col-span-2">
    <label className="block text-sm font-medium">Lieu ou adresse <span className="text-red-600">* obligatoire</span>
      <div className="relative mt-2"><MapPin className="absolute left-3 top-3.5 h-4 w-4 text-ink-400" /><input ref={inputRef} value={query} onChange={(event) => { setQuery(event.target.value); setSelected(null); setResults([]); event.target.setCustomValidity('Sélectionne une adresse dans la liste proposée.'); }} onInvalid={(event) => event.currentTarget.setCustomValidity(query ? 'Sélectionne une adresse dans la liste proposée.' : 'Indique le lieu du tournoi.')} required placeholder="Ex. Cap Homard, Saint-Paul" autoComplete="off" className="w-full rounded-xl border border-ink-200 py-3 pl-10 pr-10" />{loading && <LoaderCircle className="absolute right-3 top-3.5 h-4 w-4 animate-spin text-ink-400" />}</div>
    </label>
    ...
    <p className="mt-2 text-xs text-ink-400">Résultats OpenStreetMap · sélectionne obligatoirement une proposition. Les autres informations du formulaire seront conservées si celle-ci manque.</p>
  </div>;
}
