...
type Props = { usage: ImageUsage; maxFiles?: number; associationId?: string; required?: boolean; inputName?: string; bucket?: 'media-public' | 'branding' };

export default function OptimizedMediaPicker({ usage, maxFiles = 1, associationId, required = false, inputName = 'media_ids', bucket = 'media-public' }: Props) {
  ...
  async function choose(files: FileList | null) {
    ...
    try {
      ...
      for (const original of selected) {
        ...
        const { error: uploadError } = await supabase.storage.from(bucket).upload(path, optimized.file, { cacheControl: '31536000', contentType: 'image/webp', upsert: false });
